...
describe('POST /api/services/[name]/restart', () => {
  ...
  function createMockRequest(name: string): {
    ...
    const request = new NextRequest(`http://localhost:3000/api/services/${name}/restart`, {
      method: 'POST',
    });
    return { request, params: Promise.resolve({ name }) };
  }

  it('restarts a service successfully', async () => {
    vi.mocked(isAllowedService).mockReturnValue(true);
    vi.mocked(restartService).mockResolvedValue({ success: true });
    ...
    expect(restartService).toHaveBeenCalledWith('docker');
  });
  ...
  it('returns 500 on exception', async () => {
    vi.mocked(isAllowedService).mockReturnValue(true);
    vi.mocked(restartService).mockRejectedValue(new Error('System error'));
