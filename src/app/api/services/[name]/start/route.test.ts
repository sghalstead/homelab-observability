...
describe('POST /api/services/[name]/start', () => {
  ...
  function createMockRequest(name: string): {
    ...
    const request = new NextRequest(`http://localhost:3000/api/services/${name}/start`, {
      method: 'POST',
    });
    return { request, params: Promise.resolve({ name }) };
  }

  it('starts a service successfully', async () => {
    vi.mocked(isAllowedService).mockReturnValue(true);
    vi.mocked(startService).mockResolvedValue({ success: true });
    ...
    expect(startService).toHaveBeenCalledWith('docker');
  });
  ...
  it('returns 500 when start fails', async () => {
    vi.mocked(isAllowedService).mockReturnValue(true);
    vi.mocked(startService).mockResolvedValue({
      success: false,
  ...
  it('returns 500 on exception', async () => {
    vi.mocked(isAllowedService).mockReturnValue(true);
    vi.mocked(startService).mockRejectedValue(new Error('System error'));
